...
try {
    ...
    if (!isset($data['relationship_id']) || !isset($data['action'])) {
        throw new Exception('relationship_id and action are required');
    }
    ...
    if (isset($_SESSION['mentor_id']) || isset($_SESSION['sponsor_id'])) {
        $responder_id = $_SESSION['mentor_id'] ?? $_SESSION['sponsor_id'];
        $responder_type = 'mentor';
    } else {
        $responder_id = $_SESSION['user_id'];
        $responder_type = 'mentee';
    }

    // Respond to request
    $result = $mentorshipManager->respondToRequest($relationship_id, $action, $responder_id, $responder_type);
